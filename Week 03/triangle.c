...
int convert(const char length[]) {
    long m;
    if(valid(length)) m = atol(length);
    else m = -1;
    if(m > INT_MAX) m = -1;
    return m;
}

int distinct(long max, long other1, long other2) {
    if(max == other1 && max == other2 && other1 == other2) return Equilateral;
    else if(max == other1 || max == other2 || other1 == other2) return Isosceles;
    else if(max * max == (other1 * other1) + (other2 * other2)) return Right;
    else if(max < other1 + other2) return Scalene;
    else if(max == other1 + other2) return Flat;
    else if(max > other1 + other2) return Impossible;
    else return Illegal;
}

char compare(long a, long b, long c) {
    char n;
    long max, other1, other2;
    if (a >= b) {
        if (a >= c) max = a, other1 = b, other2 = c;
        else max = c, other1 = a, other2 = b;
    }
    else {
        if (b >= c) max = b, other1 = a, other2 = c;
        else max = c, other1 = a, other2 = b;
    }
    n = distinct(max, other1, other2);
    return n;
}
...
int triangle(int a, int b, int c) {
    ...
    long longB = (long)b;
    long longC = (long)c;
    if(longA <= 0 || longB <= 0 || longC <= 0) return Illegal;
    else return compare(longA, longB, longC);
}
