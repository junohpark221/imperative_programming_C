...
// Check whether the string satisfies BadCell or not.
int checkBad(game *g, char *text) {
    int i = 0;
    for(int a = 0; a < 3; a++) {
        for(int b = 0; b < 3; b++) {
            if(g->grid[a][b] == N) i++;
...
validity valid(game *g, char *text) {
    if(checkBad(g, text) == -1) return BadCell;
...
player line(player p0, player p1, player p2) {
    if(p0 == p1 && p0 == p2 && p1 == p2) return p0;
    else return N;
}

player checkRow(game *g) {
    player a = line(g->grid[0][0], g->grid[0][1], g->grid[0][2]);
    player b = line(g->grid[1][0], g->grid[1][1], g->grid[1][2]);
    player c = line(g->grid[2][0], g->grid[2][1], g->grid[2][2]);
    if(a != N) return a;
...
player checkCol(game *g) {
    player a = line(g->grid[0][0], g->grid[1][0], g->grid[2][0]);
    player b = line(g->grid[0][1], g->grid[1][1], g->grid[2][1]);
    player c = line(g->grid[0][2], g->grid[1][2], g->grid[2][2]);
    if(a != N) return a;
    else if(b != N) return b;
    else if(c != N) return c;
    else return N;
...
player checkDia(game *g) {
    player a = line(g->grid[0][0], g->grid[1][1], g->grid[2][2]);
    player b = line(g->grid[2][0], g->grid[1][1], g->grid[0][2]);
    if(a != N) return a;
    else if(b != N) return b;
    else return N;
}
