...
// Convert a mark string into a integer.
// It is for the string with the length of 1
int lengthIsOne(const char mark[]) {
    int num;
    num = mark[0] - '0';
    return num;
}
...
// Convert a mark string into a integer.
// It is for the string with the length of 2
int lengthIsTwo(const char mark[]) {
    int num;
    num = (mark[0] - '0') * 10 + (mark[1] - '0');
    return num;
}

// Convert a mark string into a integer.
// It is for the string with the length of 3
int lengthIsThree(const char mark[]) {
    int num;
    num = (mark[0] - '0') * 100 + (mark[1] - '0') + (mark[2] - '0');
    return num;
}
...
int convert(int n, const char mark[]) {
    int num;
    if (n == 1) num = lengthIsOne(mark);
    else if (n == 2) num = lengthIsTwo(mark);
    else if (n == 3) num = lengthIsThree(mark);
    else num = -1;
    return num;
}
...
int grade(int m) {
    if (m >= 0 && m < 40) return Fail;
    else if (m >= 40 && m < 50) return G3;
    else if (m >= 50 && m < 60) return G22;
    else if (m >= 60 && m < 70) return G21;
    else if (m >= 70 && m <= 100) return G1;
    else return Invalid;
}
